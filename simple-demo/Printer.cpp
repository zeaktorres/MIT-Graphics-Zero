...
Printer::Printer() {
  ...
  auto cb = [this](hscpp::SwapInfo &info) {
    ...
    case hscpp::SwapPhase::AfterSwap:
      ...
      // This new object is in a new dynamic library, but it still has access to
      // global data in GlobalUserData.
      SimpleDemoData *pDemoData =
          hscpp::GlobalUserData::GetAs<SimpleDemoData>();

      // After recompiling Printer.cpp, old instances of the Printer class will
      // have been deleted, so the entries in the global printers array point to
      // invalid data. Replace these instances with the newly constructed
      // Printers.
...
void Printer::Update() {
  ...
  std::cout << "Printer" << m_Name << "' has index " << m_Index << "."
            << std::endl;
}
